...
a|{set:cellbgcolor:#00FF00} *Data Science Perspective*: Predictive models are trained using the provided attributes (variables), but how does that work? 

Some variables are correlated with each other. When those attributes vary, the correlated attributes will also vary to some degree. For reasonable model performance, we need to:
