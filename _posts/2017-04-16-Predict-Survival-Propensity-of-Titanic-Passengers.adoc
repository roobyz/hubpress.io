...
Yes, for example, if someone was in a lower deck, they may not have been able to get to the lifeboats: which is correlated to class. To achieve reasonably accurate predictions, the model that we create will assign different weights to the attributes (variables of importance).
|===
